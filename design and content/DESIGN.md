...
3. **Featured materials:** a horizontal slider of up to 10 materials on the Deep Sage band. Four equal cards show at a time (three on tablets, two below 768px, one and a bit on phones) and snap into place when swiped or scrolled. Each Paper White card has a 4:3 image, a category chip, the title and a two-line summary, then "starting from" credits and the level count under a dashed rule. The heading row holds the "all materials" pill and round Deep Forest previous and next arrows that turn lime on hover and fade out at either end. A thin progress rail under the row shows how far along the slider is.
