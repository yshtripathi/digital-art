...
<x-breadcrumb :title="__('common.checkout')" />
        ...
        <form name="frmCheckout" id="frmCheckout" method="POST" action="{{ route('cart.order') }}">
            ...
            <div class="co-grid">
                ...
                <div class="co-main">
                    ...
                    {{-- Billing --}}
                    <div class="co-card">
                        <h3 class="co-card__title"><i class="fas fa-user-circle"></i> {{ __('common.billing_details') }}</h3>
                        <div class="co-fields">
                            <div class="co-field">
                                <label class="co-label">{{ __('common.first_name') }} *</label>
                                <input type="text" name="first_name" id="first_name" value="" placeholder="{{ __('common.first_name') }}" class="co-input">
                                @error('first_name')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.last_name') }} *</label>
                                <input type="text" name="last_name" id="last_name" value="" placeholder="{{ __('common.last_name') }}" class="co-input">
                                @error('last_name')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.email') }} *</label>
                                <input name="email" type="email" id="email" value="{{ auth()->user()->email ?? '' }}" placeholder="{{ __('common.email') }}" class="co-input">
                                @error('email')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.phone') }} *</label>
                                <input type="tel" name="phone" id="phone" placeholder="{{ __('common.phone') }}" value="{{ auth()->user()->phone ?? '' }}" class="co-input" pattern="[0-9\-\+\s\(\)]{7,}" inputmode="tel">
                                @error('phone')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field co-field--full">
                                <label class="co-label">{{ __('common.address') }} *</label>
                                <input type="text" name="address1" id="address" value="{{ auth()->user()->address ?? '' }}" placeholder="{{ __('common.address') }}" class="co-input">
                                @error('address1')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.town_city') }} *</label>
                                <input type="text" name="city" id="city" value="{{ auth()->user()->city ?? '' }}" placeholder="{{ __('common.town_city') }}" class="co-input">
                                @error('city')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.zip_code') }} *</label>
                                <input type="text" name="post_code" id="post_code" pattern="[0-9]*" placeholder="{{ __('common.zip_code') }}" value="{{ auth()->user()->zip ?? '' }}" class="co-input">
                                @error('post_code')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.state') }} *</label>
                                <input type="text" name="state" id="state" value="{{ auth()->user()->state ?? '' }}" placeholder="{{ __('common.state') }}" class="co-input">
                                @error('state')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.country') }} *</label>
                    ...
                    {{-- Additional info --}}
                    <div class="co-card">
                        <h3 class="co-card__title"><i class="fas fa-clipboard-list"></i> {{ __('common.additional_information') }}</h3>
                        <div class="co-field co-field--full">
                            <label class="co-label">{{ __('common.notes') }}</label>
                            <textarea name="note" placeholder="{{ __('common.notes_about_order') }}" class="co-input co-textarea"></textarea>
                        </div>
                    </div>

                    {{-- Card details --}}
                    <div class="co-card">
                        <h3 class="co-card__title"><i class="fas fa-credit-card"></i> {{ __('common.card_details') }}</h3>
                        <div class="co-fields">
                            <div class="co-field co-field--full">
                                <label class="co-label">{{ __('common.card_holder_name') }}</label>
                                <input type="text" name="name" id="name_on_card" class="co-input" placeholder="{{ __('common.card_holder_name') }}">
                                @error('name')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field co-field--full">
                                <label class="co-label">{{ __('common.card_number') }}</label>
                                <input type="text" name="card_number" id="card_number" placeholder="{{ __('common.card_number_placeholder') }}" class="co-input cc-number" pattern="[0-9\s]{19}" inputmode="numeric" maxlength="19" autocomplete="cc-number">
                                @error('card_number')<span class="co-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="co-field">
                                <label class="co-label">{{ __('common.expiry_month') }}</label>
                                <div class="co-expiry">
                            ...
                            <div class="co-field">
                                <label class="co-label">{{ __('common.cvv') }}</label>
                    ...
                    {{-- Terms --}}
                    <div class="co-card">
                        <h3 class="co-card__title"><i class="fas fa-file-signature"></i> {{ __('common.terms_conditions') }}</h3>

                        <div class="co-field co-field--check">
                            <label class="co-check"><input type="checkbox" id="terms" name="terms"><span>{{ __('common.agree_terms_conditions') }} <a href="{{ route('pages', 'terms-conditions') }}" target="_blank">{{ __('common.terms_conditions') }}</a></span></label>
                            @error('terms')<span class="co-error">{{ $message }}</span>@enderror
                        </div>
                        <div class="co-field co-field--check">
                            <label class="co-check"><input type="checkbox" id="privacy" name="privacy"><span>{{ __('common.agree_privacy_policy') }} <a href="{{ route('pages', 'privacy-policy') }}" target="_blank">{{ __('common.privacy_policy') }}</a></span></label>
                            @error('privacy')<span class="co-error">{{ $message }}</span>@enderror
                        </div>
                        <div class="co-field co-field--check">
                            <label class="co-check"><input type="checkbox" id="delivery" name="delivery"><span>{{ __('common.agree_delivery_policy') }} <a href="{{ route('pages', 'delivery-policy') }}" target="_blank">{{ __('common.delivery_policy') }}</a></span></label>
                            @error('delivery')<span class="co-error">{{ $message }}</span>@enderror
                        </div>
                        <div class="co-field co-field--check">
                            <label class="co-check"><input type="checkbox" id="refund" name="refund"><span>{{ __('common.agree_refund_policy') }} <a href="{{ route('pages', 'refund-policy') }}" target="_blank">{{ __('common.refund_policy') }}</a></span></label>
                            @error('refund')<span class="co-error">{{ $message }}</span>@enderror
                        </div>

                        <div class="co-billnote">
                            <p>{{ __('common.card_bill_description') }}</p>
                ...
                <aside class="co-aside">
                    <div class="co-card co-summary">
                        <h3 class="co-card__title"><i class="fas fa-shopping-bag"></i> {{ __('common.your_order') }}</h3>
                            ...
                            @if(Helper::getAllProductFromCart())
                                @foreach(Helper::getAllProductFromCart() as $key => $cart)
                                    ...
                                    <div class="co-order__item">
                                        <span class="co-order__points"><i class="fas fa-coins"></i> {{ number_format($points, 0, '.', ',') }} {{ __('common.points') }}</span>
                        ...
                        @if(env('CAPTCHA_ENABLED', true))
                            <div class="co-field co-field--captcha">
                                <label class="co-label">{{ __('common.security_code') }} *</label>
                                <div class="co-captcha">
                                    <input type="text" id="captcha" name="captcha" autocomplete="off" placeholder="{{ __('common.fill_captcha') }}" class="co-input">
                                    <div class="co-captcha__img">@captcha</div>
                                </div>
                                @error('captcha')<span class="co-error">{{ __('common.captcha_error') }}</span>@enderror
                            </div>
                        @endif

                        <button type="submit" class="co-btn co-btn--primary" id="button-confirm"><i class="fas fa-shield-alt"></i> {{ __('common.place_order') }}</button>
                        <a href="{{ route('home') }}" class="co-btn co-btn--ghost"><i class="fas fa-arrow-left"></i> {{ __('common.continue_shopping') }}</a>
