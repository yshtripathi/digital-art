...
@include('frontend.layouts.breadcrumb', [
    'title' => __('inkwave.order_ok_title'),
    'links' => [
        ['name' => __('inkwave.menu_home'), 'url' => route('home')],
        ['name' => __('inkwave.order_ok_title')]
    ]
])
...
<div class="duo-order-wrapper">
    <div class="duo-container">
        <div class="duo-order-card">
            ...
            <h1 class="duo-order-title">{{ __('inkwave.order_ok_heading') }}</h1>
            <p class="duo-order-sub">{{ __('inkwave.order_ok_msg') }}</p>
            ...
            @if($order)
                ...
                <div class="duo-order-details">
                    <div class="duo-order-detail">
                        <span class="duo-order-label">{{ __('inkwave.order_ok_number') }}</span>
                        <span class="duo-order-value">{{ $order->order_number }}</span>
                    </div>
                    <div class="duo-order-detail">
                        <span class="duo-order-label">{{ __('inkwave.order_ok_amount') }}</span>
                        <span class="duo-order-value">{{ $currency }} {{ number_format($order->total_amount, $order->currency == 'JPY' ? 0 : 2) }}</span>
                    </div>
                    <div class="duo-order-detail">
                        <span class="duo-order-label">{{ __('inkwave.order_ok_trans') }}</span>
                        <span class="duo-order-value">{{ $transaction_id }}</span>
                    </div>
                    <div class="duo-order-detail">
                        <span class="duo-order-label">{{ __('inkwave.order_ok_status') }}</span>
            ...
            <div class="duo-order-actions">
                @if($order)
                    <a href="{{ route('user.order.show', $order->id) }}" class="duo-btn duo-btn--primary"><i class="fas fa-eye"></i> {{ __('inkwave.order_ok_view') }}</a>
                @endif
                <a href="{{ route('home') }}" class="duo-btn duo-btn--ghost"><i class="fas fa-home"></i> {{ __('inkwave.order_ok_home') }}</a>
                @if($order)
                    <a href="{{ route('order.pdf', $order->id) }}" class="duo-btn duo-btn--ghost"><i class="fas fa-download"></i> {{ __('inkwave.order_ok_invoice') }}</a>
                @endif
            </div>

            @if($email_status == 'inactive' && $order)
                <p class="duo-note">{{ __('inkwave.order_ok_note') }} <a href="{{ route('order.pdf', $order->id) }}">{{ __('inkwave.order_ok_invoice') }}</a></p>
            @endif
