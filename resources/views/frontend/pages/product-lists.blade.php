...
<div class="ag-catalog-page">
    ...
    <div class="ag-catalog-header">
        @if($isCat)
            ...
            @endif
        @else
            <h1>{{ __('managenovax.catalog.browse') }}</h1>
            <p>{{ __('managenovax.catalog.explore_desc') }}</p>
        @endif
    ...
    <div class="ag-container">
        ...
        {{-- ================= PRODUCT GRID ================= --}}
        <h2 class="ag-section-title">{{ $products->count() }} {{ __('managenovax.catalog.items') }}</h2>
                        ...
                        <div class="ag-product-card__body">
                            ...
                            @if($course->levels && $course->levels->count() > 0)
                                <p class="ag-product-card__price">
                                    {{ __('managenovax.catalog.starting_from') }} 
                                    <strong><i class="fas fa-coins"></i> {{ number_format($course->levels->min('price_in_points')) }}</strong> 
                                    {{ __('managenovax.catalog.credits_label') }}
                                </p>
                            @else
                                <p class="ag-product-card__price">
                                    <strong>{{ __('managenovax.catalog.free_label') }}</strong>
                                </p>
                            @endif
                            
                            <div class="ag-primary-btn">{{ __('managenovax.catalog.view_btn') }}</div>
                        </div>
                    </a>
                @endforeach
            </div>
        ...
        @else
            <div class="ag-empty-state">
                ...
                <h3>{{ __('managenovax.catalog.no_products') }}</h3>
                <p>{{ __('managenovax.catalog.explore_desc') }}</p>
            </div>
        @endif
        ...
        @if($allCategories->count())
            <div style="margin-top: 80px;">
                <h2 class="ag-section-title">{{ __('managenovax.catalog.other_cats') }}</h2>
