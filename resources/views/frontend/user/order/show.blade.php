        ...
        @if($order)
            ...
            <div class="ag-receipt-card">
                ...
                <div class="ag-receipt-head">
                    <div>
                        <span class="ag-receipt-eyebrow">{{ __('managenovax.receipt.doc_title') }}</span>
                ...
                <div class="ag-receipt-summary">
                    <div class="ag-summary-item">
                        <span class="ag-summary-label">{{ __('managenovax.receipt.lbl_total') }}</span>
                        <span class="ag-summary-val">{!! $totalFmt !!}</span>
                    </div>
                    <div class="ag-summary-item">
                        <span class="ag-summary-label">{{ __('managenovax.receipt.lbl_status') }}</span>
                        <span class="ag-receipt-tag ag-receipt-tag--solid">{{ ucwords($order->status) }}</span>
                    </div>
                    <div class="ag-summary-item">
                        <span class="ag-summary-label">{{ __('managenovax.receipt.lbl_payment') }}</span>
                        <span class="ag-receipt-tag"><i class="fas fa-check-circle"></i> {{ ucwords($order->payment_status) }}</span>
                    </div>
                </div>

                {{-- Details --}}
                <h2 class="ag-receipt-h"><i class="fas fa-clipboard-list"></i> {{ __('managenovax.receipt.hdg_info') }}</h2>
                
                <div class="ag-details-grid">
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('managenovax.receipt.lbl_ref') }}</span>
                        <span class="ag-detail-value">{{ $order->order_number }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('managenovax.receipt.lbl_name') }}</span>
                        <span class="ag-detail-value">{{ $order->first_name }} {{ $order->last_name }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('managenovax.receipt.lbl_email') }}</span>
                        <span class="ag-detail-value">{{ $order->email }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('managenovax.receipt.lbl_date') }}</span>
                        <span class="ag-detail-value">{{ $order->created_at->format('D d M, Y') }} {{ __('managenovax.receipt.at_time') }} {{ $order->created_at->format('g:i a') }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('managenovax.receipt.lbl_qty') }}</span>
                        <span class="ag-detail-value">{{ $order->quantity }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('managenovax.receipt.lbl_method') }}</span>
                        <span class="ag-detail-value"><i class="far fa-credit-card"></i> {{ __('managenovax.receipt.lbl_card') }}</span>
                    </div>
                    <div class="ag-detail-field ag-detail-field--wide">
                        <span class="ag-detail-label">{{ __('managenovax.receipt.lbl_trans_id') }}</span>
