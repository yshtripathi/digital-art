...
@section('title', __('inkwave.receipt_doc_title'))

@section('main-content')

@include('frontend.layouts.breadcrumb', [
    'title' => __('inkwave.receipt_doc_title'),
    'links' => [
        ['name' => __('inkwave.top_nav_home'), 'url' => route('home')],
        ['name' => __('inkwave.userdash_my_account'), 'url' => route('user')],
        ['name' => __('inkwave.receipt_doc_title')]
    ]
])
        ...
        @if($order)
            ...
            <div class="ag-receipt-card">
                ...
                <div class="ag-receipt-head">
                    <div>
                        <span class="ag-receipt-eyebrow">{{ __('inkwave.receipt_doc_title') }}</span>
                    ...
                    <div class="ag-receipt-actions">
                        <button type="button" onclick="window.history.back();" class="ag-receipt-btn ag-receipt-btn--ghost">
                            <i class="fas fa-arrow-left"></i> {{ __('inkwave.receipt_btn_back') }}
                        </button>
                        <a href="{{ route('order.pdf', $order->id) }}" class="ag-receipt-btn ag-receipt-btn--primary">
                            <i class="fas fa-download"></i> {{ __('inkwave.receipt_btn_pdf') }}
                        </a>
                    </div>
                </div>
                ...
                <div class="ag-receipt-summary">
                    <div class="ag-summary-item">
                        <span class="ag-summary-label">{{ __('inkwave.receipt_lbl_total') }}</span>
                        <span class="ag-summary-val">{!! $totalFmt !!}</span>
                    </div>
                    <div class="ag-summary-item">
                        <span class="ag-summary-label">{{ __('inkwave.receipt_lbl_status') }}</span>
                        <span class="ag-receipt-tag ag-receipt-tag--solid">{{ ucwords($order->status) }}</span>
                    </div>
                    <div class="ag-summary-item">
                        <span class="ag-summary-label">{{ __('inkwave.receipt_lbl_payment') }}</span>
                        <span class="ag-receipt-tag"><i class="fas fa-check-circle"></i> {{ ucwords($order->payment_status) }}</span>
                    </div>
                </div>

                {{-- Details --}}
                <h2 class="ag-receipt-h"><i class="fas fa-clipboard-list"></i> {{ __('inkwave.receipt_hdg_info') }}</h2>
                
                <div class="ag-details-grid">
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('inkwave.receipt_lbl_ref') }}</span>
                        <span class="ag-detail-value">{{ $order->order_number }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('inkwave.receipt_lbl_name') }}</span>
                        <span class="ag-detail-value">{{ $order->first_name }} {{ $order->last_name }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('inkwave.receipt_lbl_email') }}</span>
                        <span class="ag-detail-value">{{ $order->email }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('inkwave.receipt_lbl_date') }}</span>
                        <span class="ag-detail-value">{{ $order->created_at->format('D d M, Y') }} {{ __('inkwave.receipt_at_time') }} {{ $order->created_at->format('g:i a') }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('inkwave.receipt_lbl_qty') }}</span>
                        <span class="ag-detail-value">{{ $order->quantity }}</span>
                    </div>
                    <div class="ag-detail-field">
                        <span class="ag-detail-label">{{ __('inkwave.receipt_lbl_method') }}</span>
                        <span class="ag-detail-value"><i class="far fa-credit-card"></i> {{ __('inkwave.receipt_lbl_card') }}</span>
                    </div>
                    <div class="ag-detail-field ag-detail-field--wide">
                        <span class="ag-detail-label">{{ __('inkwave.receipt_lbl_trans_id') }}</span>
