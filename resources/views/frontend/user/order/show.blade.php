...
<x-breadcrumb :title="__('common.order_detail')" />
    ...
    <div class="od-container">
        @if($order)
            ...
            <div class="od-card">
                ...
                <div class="od-head">
                    <div>
                        <p class="od-eyebrow">{{ __('common.order_detail') }}</p>
                        <h1 class="od-num">#{{ $order->order_number }}</h1>
                    </div>
                    <div class="od-actions">
                        <button type="button" onclick="window.history.back();" class="od-btn od-btn--ghost"><i class="fas fa-arrow-left"></i> {{ __('common.back') }}</button>
                        <a href="{{ route('order.pdf', $order->id) }}" class="od-btn od-btn--primary"><i class="fas fa-download"></i> {{ __('common.generate_pdf') }}</a>
                    </div>
                </div>
                ...
                <div class="od-summary">
                    <div class="od-summary__item">
                        <span class="od-summary__label">{{ __('common.total_amount') }}</span>
                        <span class="od-summary__amt">{{ $totalFmt }}</span>
                    </div>
                    <div class="od-summary__item">
                        <span class="od-summary__label">{{ __('common.order_status') }}</span>
                        <span class="od-tag od-tag--solid">{{ ucwords($order->status) }}</span>
                    </div>
                    <div class="od-summary__item">
                        <span class="od-summary__label">{{ __('common.payment_status') }}</span>
                        <span class="od-tag">{{ ucwords($order->payment_status) }}</span>
                    </div>
                </div>

                {{-- Details --}}
                <h2 class="od-h">{{ __('common.order_information') }}</h2>
                <div class="od-grid">
                    <div class="od-field">
                        <span class="od-field__label">{{ __('common.order_number') }}</span>
                        <span class="od-field__value">{{ $order->order_number }}</span>
                    </div>
                    <div class="od-field">
                        <span class="od-field__label">{{ __('common.name') }}</span>
                        <span class="od-field__value">{{ $order->first_name }} {{ $order->last_name }}</span>
                    </div>
                    <div class="od-field">
                        <span class="od-field__label">{{ __('common.email') }}</span>
                        <span class="od-field__value">{{ $order->email }}</span>
                    </div>
                    <div class="od-field">
                        <span class="od-field__label">{{ __('common.order_date') }}</span>
                        <span class="od-field__value">{{ $order->created_at->format('D d M, Y') }} {{ __('common.at_time') }} {{ $order->created_at->format('g:i a') }}</span>
                    </div>
                    <div class="od-field">
                        <span class="od-field__label">{{ __('common.quantity') }}</span>
                        <span class="od-field__value">{{ $order->quantity }}</span>
                    </div>
                    <div class="od-field">
                        <span class="od-field__label">{{ __('common.payment_method') }}</span>
                        <span class="od-field__value">{{ __('common.credit_card') }}</span>
                    </div>
                    <div class="od-field od-field--wide">
                        <span class="od-field__label">{{ __('common.transaction_id') }}</span>
