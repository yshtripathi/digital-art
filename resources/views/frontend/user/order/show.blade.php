...
@include('frontend.layouts.breadcrumb', [
    'title' => __('inkwave.od_title'),
    'links' => [
        ['name' => __('inkwave.menu_home'), 'url' => route('home')],
        ['name' => __('inkwave.dash_my_account'), 'url' => route('user')],
        ['name' => __('inkwave.od_title')]
    ]
])
        ...
        @if($order)
            ...
            <div class="duo-od-card">
                ...
                <div class="duo-od-head">
                    <div>
                        <p class="duo-od-eyebrow">{{ __('inkwave.od_title') }}</p>
                        <h1 class="duo-od-num">#{{ $order->order_number }}</h1>
                    </div>
                    <div class="duo-od-actions">
                        <button type="button" onclick="window.history.back();" class="duo-od-btn duo-od-btn--ghost"><i class="fas fa-arrow-left"></i> {{ __('inkwave.od_back') }}</button>
                        <a href="{{ route('order.pdf', $order->id) }}" class="duo-od-btn duo-od-btn--primary"><i class="fas fa-download"></i> {{ __('inkwave.od_generate_pdf') }}</a>
                    </div>
                </div>
                ...
                <div class="duo-od-summary">
                    <div class="duo-od-summary__item">
                        <span class="duo-od-summary__label">{{ __('inkwave.od_total_amount') }}</span>
                        <span class="duo-od-summary__amt">{{ $totalFmt }}</span>
                    </div>
                    <div class="duo-od-summary__item">
                        <span class="duo-od-summary__label">{{ __('inkwave.od_order_status') }}</span>
                        <span class="duo-od-tag duo-od-tag--solid">{{ ucwords($order->status) }}</span>
                    </div>
                    <div class="duo-od-summary__item">
                        <span class="duo-od-summary__label">{{ __('inkwave.od_payment_status') }}</span>
                ...
                {{-- Details --}}
                <h2 class="duo-od-h"><i class="fas fa-clipboard-list"></i> {{ __('inkwave.od_order_information') }}</h2>
                <div class="duo-od-grid">
                    <div class="duo-od-field">
                        <span class="duo-od-field__label">{{ __('inkwave.od_order_number') }}</span>
                        <span class="duo-od-field__value">{{ $order->order_number }}</span>
                    </div>
                    <div class="duo-od-field">
                        <span class="duo-od-field__label">{{ __('inkwave.od_name') }}</span>
                        <span class="duo-od-field__value">{{ $order->first_name }} {{ $order->last_name }}</span>
                    </div>
                    <div class="duo-od-field">
                        <span class="duo-od-field__label">{{ __('inkwave.od_email') }}</span>
                        <span class="duo-od-field__value">{{ $order->email }}</span>
                    </div>
                    <div class="duo-od-field">
                        <span class="duo-od-field__label">{{ __('inkwave.od_order_date') }}</span>
                        <span class="duo-od-field__value">{{ $order->created_at->format('D d M, Y') }} {{ __('inkwave.od_at_time') }} {{ $order->created_at->format('g:i a') }}</span>
                    </div>
                    <div class="duo-od-field">
                        <span class="duo-od-field__label">{{ __('inkwave.od_quantity') }}</span>
                        <span class="duo-od-field__value">{{ $order->quantity }}</span>
                    </div>
                    <div class="duo-od-field">
                        <span class="duo-od-field__label">{{ __('inkwave.od_payment_method') }}</span>
                        <span class="duo-od-field__value"><i class="far fa-credit-card"></i> {{ __('inkwave.od_credit_card') }}</span>
                    </div>
                    <div class="duo-od-field duo-od-field--wide">
                        <span class="duo-od-field__label">{{ __('inkwave.od_transaction_id') }}</span>
